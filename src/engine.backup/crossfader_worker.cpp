...
void CrossfaderWorker::timerTickPre()
{
    ...
    {
        --m_ticks_fade_out;
        unsigned int new_x = (MASTER_MAX_VALUE * m_ticks_fade_out / m_total_ticks_fade_out);
    ...
    {
        --m_ticks_fade_in;
        unsigned int new_y = MASTER_MAX_VALUE - (MASTER_MAX_VALUE * m_ticks_fade_in / m_total_ticks_fade_in);
    ...
    if (m_xy.getLevelX() == 0 && m_xy.getLevelY() == MASTER_MAX_VALUE) {
...
void CrossfaderWorker::timerTickPerChannel(crossfader_channel_t & channel)
{
    ...
    if (channel.p_channel->noFade())
    {
        ...
        channel.output_x = channel.min + ( channel.delta_min_x * m_xy.getLevelX() / MASTER_MAX_VALUE);
        channel.output_y = channel.min + ( channel.delta_min_y * m_xy.getLevelY() / MASTER_MAX_VALUE);
...
*/
    ...
    } else if ((m_xy.getLevelX() == MASTER_MAX_VALUE) && (m_xy.getLevelY() == 0)) {
