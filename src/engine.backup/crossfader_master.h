...
{
    public:
        CrossfaderMaster() : NotifyingMaster(MASTER_MAX_VALUE), m_mode(MODE_SPLIT), m_x_or_xy(NULL), m_y(NULL),
                                 m_reversed(false), m_level_x(MASTER_MAX_VALUE), m_level_y(0) { LOG_DEBUG("XFADER-MASTER: CONSTRUCTOR this object " << this); };
        ~CrossfaderMaster() { LOG_DEBUG("XFADER-MASTER: DESTRUCTOR"); };

        inline const unsigned int getPhyLevelX() const
            { return (m_reversed ? MASTER_MAX_VALUE - m_level_x : m_level_x); };
        inline const unsigned int getPhyLevelY() const
            { return (m_reversed ? m_level_y : MASTER_MAX_VALUE - m_level_y); };
        ...
        inline void toggleReversed() { LOG_DEBUG("XFADER-MASTER: Toggling Reversed"); m_reversed = !m_reversed; m_level_x = MASTER_MAX_VALUE-m_level_x ;
                                       m_level_y = MASTER_MAX_VALUE - m_level_y; };
